...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences, vocab):
        ...
        self.enc_len = len(article_words)  # 在截断之后但在填充之前保存长度
        ...
        if config.pointer_gen:
            # 保存正文在整个扩展词表的ID和临时的未登录词表中的ID
            self.enc_input_extend_vocab, self.article_oovs = data.article2ids(article_words, vocab)

            # 保存标注标题在整个扩展词表的ID
            abs_ids_extend_vocab = data.abstract2ids(abstract_words, vocab, self.article_oovs)

            # 覆盖解码器目标序列，以便使用临时正文的OOV（未登录词）ID
            _, self.target = self.get_dec_inp_targ_seqs(abs_ids_extend_vocab,
                                                        config.max_dec_steps, start_decoding,
                                                        stop_decoding)
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list):
        ...
        # 注意：我们的enc_batch对于每个批处理可以有不同的长度（第二维），因为我们对编码器使用dynamic_rnn。
